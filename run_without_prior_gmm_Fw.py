...
def main():
    ...
    mask = zz[ixx] < -700
    ah = data['BR_Anhydrite'][:,0] # correlates to "anhydrite index" derived from hyperspectral 
    ...
    ###########################################################################
    H = position_cord.shape[0] # get the shape of groud truth
    n_features = 2 # get the number of features including co-ordinates and label
    
    # Create a dataset which has "X","Y","Z", "Label", and spectral channel information
    data_hsi = torch.zeros((H, n_features))
    for i in range(H):
        data_hsi[i,0] = position_cord[i]
        data_hsi[i,1] = hsi_data[i]
    ...
    ###########################################################################
    normalised_hsi = data_hsi[:,1]
    gm = BayesianGaussianMixture(n_components=3, random_state=0).fit(normalised_hsi.reshape(-1,1))
    
    # make the labels to start with 1 instead of 0
    gmm_label = gm.predict(normalised_hsi.reshape(-1,1)) +1 
    print(gmm_label)
    exit()
    ...
    if plot_dimred=="tsne":
        ...
        TSNE_transformation(data=normalised_data, label= np.round(custom_grid_values_post), filename="./Results_without_prior_gmm/tsne_gempy_final_label.png")
