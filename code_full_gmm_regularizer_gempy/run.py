...
parser.add_argument('--alpha', metavar='alpha', type=float , default=10, help='scaling parameter for the mean, 0.1')
parser.add_argument('--beta', metavar='beta', type=float , default= 1, help='scaling parameter for the covariance, 20')
parser.add_argument('--reg', metavar='reg', type=float , default= 1e-2, help='regularizer to force posterior of GMM to move toward gempy')
dtype = torch.float64
...
def main():
    ...
    if posterior_condition==1:
        directory_path = directory_path + "/" + dataset + "/posterior_condition_" + str(posterior_condition) + "_slope_gempy_" + str(slope_gempy) +"_reg_"+ str(reg)+ "_scale_" + str(scale) +"_chain_" + str(posterior_num_chain) + "_warmup_step_" + str(posterior_warmup_steps) + "_posterior_sample_" + str(posterior_number_samples)
    elif posterior_condition==2:
        directory_path = directory_path + "/" + dataset + "/posterior_condition_" + str(posterior_condition) + "_alpha_" + str(alpha) + "_slope_gempy_" + str(slope_gempy) +"_reg_"+ str(reg)+ "_scale_" + str(scale) + "_chain_" + str(posterior_num_chain)+ "_warmup_step_" + str(posterior_warmup_steps) + "_posterior_sample_" + str(posterior_number_samples)
    elif (posterior_condition==3) or (posterior_condition==4):
        directory_path = directory_path + "/" + dataset + "/posterior_condition_" + str(posterior_condition) + "_alpha_" + str(alpha) + "_beta_"+str(beta) + "_slope_gempy_" + str(slope_gempy) +"_reg_"+ str(reg)+ "_scale_" + str(scale) +"_chain_" + str(posterior_num_chain)+ "_warmup_step_" + str(posterior_warmup_steps) + "_posterior_sample_" + str(posterior_number_samples)
    ...
    #     print("eigen_values", np.linalg.eigvals(cov_init[i]))
    ...
    test_list=[]
    if dataset=="Salinas":
        test_list.append({"update":"interface_data","id":torch.tensor([1]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[1,2],dtype=dtype, device=device), "std":torch.tensor(0.2,dtype=dtype, device=device)}})
        test_list.append({"update":"interface_data","id":torch.tensor([4]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[4,2],dtype=dtype, device=device), "std":torch.tensor(0.2,dtype=dtype, device=device)}})
        test_list.append({"update":"interface_data","id":torch.tensor([7]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[7,2],dtype=dtype, device=device), "std":torch.tensor(0.2,dtype=dtype, device=device)}})
        test_list.append({"update":"interface_data","id":torch.tensor([12]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[12,2],dtype=dtype, device=device), "std":torch.tensor(0.2,dtype=dtype, device=device)}})
    elif dataset =="KSL":
    ...
    #dot = pyro.render_model(model.model_test, model_args=(normalised_hsi,test_list,geo_model_test,mean_init,cov_init,factor,num_layers,posterior_condition, scale, cluster, alpha, beta,reg, dtype, device),render_distributions=True,filename=filename_Bayesian_graph)
    dot = pyro.render_model(model.model_test, model_args=(normalised_hsi,test_list,geo_model_test,mean_init,cov_init,factor,num_layers,posterior_condition, scale, cluster, alpha, beta, reg, dtype,device))
    
    ################################################################################
    # Prior
    ################################################################################
    pyro.set_rng_seed(42)
    prior = Predictive(model.model_test, num_samples=prior_number_samples)(normalised_hsi,test_list,geo_model_test,mean_init,cov_init,factor,num_layers,posterior_condition, scale, cluster, alpha, beta, reg, dtype,device)
    # Key to avoid
    avoid_key =[]
    ...
    #nuts_kernel = RandomWalkKernel(model=model.model_test, init_step_size=0.1, init_strategy=init_to_mean)
    ...
    mcmc.run(normalised_hsi,test_list,geo_model_test,mean_init,cov_init,factor,num_layers,posterior_condition,scale, cluster, alpha, beta,reg, dtype,device)
    ...
    for sample in zip(*posterior_samples.values()):  # Iterate over samples of all parameters
        ...
        trace = poutine.trace(lambda: model.model_test(normalised_hsi,test_list,geo_model_test,mean_init,cov_init,factor,num_layers,posterior_condition,scale, cluster, alpha, beta,reg, dtype,device)).get_trace()
        ...
        trace = poutine.trace(replayed_model).get_trace(normalised_hsi,test_list,geo_model_test,mean_init,cov_init,factor,num_layers,posterior_condition,scale, cluster, alpha, beta,reg,dtype,device)
        # Calculate the log joint (which includes log likelihood and log prior)
    ...
    posterior_predictive = Predictive(model.model_test, posterior_samples )(normalised_hsi,test_list,geo_model_test,mean_init,cov_init,factor,num_layers,posterior_condition,scale, cluster, alpha, beta,reg,dtype,device)
    plt.figure(figsize=(8,10))
    ...
    #########################################################################################
    z_nk = F.softmax(-scale* (torch.linspace(1,cluster,cluster, dtype=dtype, device=device) - torch.tensor(custom_grid_values_test.detach().numpy()).reshape(-1,1))**2, dim=1)
    ...
    if posterior_condition==1:
        ...
        cov_tensor = factor * torch.stack(cov,dim=0)
        ...
        for j in range(normalised_hsi.shape[0]):
            ...
            for c in range(cluster):
                likelihood += z_nk[j,c] * torch.exp(dist.MultivariateNormal(loc=mean_tensor[c], covariance_matrix=cov_tensor[c]).log_prob(normalised_hsi[j]))            
            for k in range(gamma_post.shape[1]):
                gamma_post[j][k] = (z_nk[j,k] * torch.exp(dist.MultivariateNormal(loc=mean_tensor[k],covariance_matrix= cov_tensor[k]).log_prob(normalised_hsi[j]))) / likelihood
    ...
    #########################################################################################
        ...
        for idx, (key, value) in enumerate(RV_mean_data_post_MAP.items()):
            mean_k = value
            eigen_vectors_data = torch.tensor(eigen_vector_list[idx], dtype=dtype, device=device)
            cov_data = eigen_vectors_data @ (torch.diag(RV_post_cov_eigen_MAP[f"cov_eigen_values_{idx+1}"+"_post"])**2 +1e-6 * torch.eye(eigen_values_list[0].shape[0], dtype=dtype, device=device)) @ eigen_vectors_data.T
            mean.append(mean_k)
            cov.append(cov_data)
    ...
    #########################################################################################
        ...
        for idx, (key, value) in enumerate(RV_mean_data_post_Mean.items()):
            mean_k = value
            eigen_vectors_data = torch.tensor(eigen_vector_list[idx], dtype=dtype, device=device)
            cov_data = eigen_vectors_data @ (torch.diag(RV_post_cov_eigen_Mean[f"cov_eigen_values_{idx+1}"+"_post"])**2 + 1e-6 * torch.eye(eigen_values_list[0].shape[0], dtype=dtype, device=device)) @ eigen_vectors_data.T
            mean.append(mean_k)
            cov.append(cov_data)
