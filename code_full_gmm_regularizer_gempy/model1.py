...
class MyModel(PyroModule):
    ...
    #@config_enumerate
    def model_test(self, obs_data,interpolation_input_,geo_model_test,mean_init,cov_init,factor,num_layers,posterior_condition, scale, cluster, alpha, beta, reg, dtype, device):
            """
            ...
            if posterior_condition==1:
                ...
                for i in range(loc_mean.shape[0]):
                    mean.append(loc_mean[i])
                    cov.append(loc_cov[i])
            ...
            if posterior_condition==3:
                ...
                for i in range(loc_mean.shape[0]):
                    ...
                    mean.append(mean_data)
                    eigen_values_init = torch.tensor(eigen_values_list[i],dtype=dtype, device =device)
                    eigen_vectors_data = torch.tensor(eigen_vector_list[i], dtype=dtype, device =device)
            ...
            if posterior_condition==4:
                ...
                for i in range(loc_mean.shape[0]):
                    ...
                    upper_tri_cov = pyro.sample("upper_tri_cov_"+str(i+1), dist.MultivariateNormal(loc=torch.zeros(num_upper_tri_elements, dtype=dtype, device =device), covariance_matrix=cov_matrix_cov))
            ...
            # We need to calculate the GMM responsibility
            
            
           
            # with pyro.plate("data", obs_data.shape[0]):
            # Compute log probabilities for all clusters in parallel
            gmm_probs = torch.exp(dist.MultivariateNormal(mean_tensor, cov_tensor).log_prob(obs_data.unsqueeze(1)))  # (N, K)
            
            pi_gmm_probs = pi.unsqueeze(0) * gmm_probs
            pi_gmm_probs_sum = torch.sum(pi_gmm_probs, axis=1)
            gamma = pi_gmm_probs/ pi_gmm_probs_sum.unsqueeze(1)
            
            log_likelihood = torch.log(pi_gmm_probs_sum)
            
            error_label = torch.sum(torch.sqrt(torch.sum((gamma - z_nk)**2, axis=1) + 1e-8))
            
            
            # Always write pyro.factor outside the pyro plate, otherwise the likelihood will be multiplied with number of plate dim
            pyro.factor("log_likelihood", log_likelihood.sum() - reg * error_label)  # Scalar log joint 
