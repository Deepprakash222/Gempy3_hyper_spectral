...
def main():
    ...
    if posterior_condition==1:
        ...
        for k in range(z_nk.shape[1]):
                ...
                cov_k = torch.zeros((mean_k.shape[0],mean_k.shape[0]), dtype=dtype, device=device)
                ...
                mean.append(mean_k)
                cov_k=cov_k/N_k[k] #+ 1e-3 * torch.diag(torch.ones(cov_k.shape[0],dtype=dtype, device=device))
                cov.append(cov_k)
    ...
    #########################################################################################
        ...
        for idx, (key, value) in enumerate(RV_mean_data_post_MAP.items()):
            D = value.shape[0]
            mean_k = value
            cov_k = torch.zeros((D,D),dtype=dtype, device=device)
            for j in range(z_nk.shape[0]):
                    cov_k +=  z_nk[j,idx]* torch.matmul((normalised_hsi[j,:] - mean_k).reshape((-1,1)) ,(normalised_hsi[j,:] - mean_k).reshape((1,-1)))
            cov_k=cov_k/N_k[idx] #+ 1e-3 * torch.diag(torch.ones(cov_k.shape[0],dtype=dtype, device=device))
            mean.append(mean_k)
            cov.append(cov_k)
        mean_tensor =torch.stack(mean,dim=0)
        cov_tensor = torch.stack(cov,dim=0)
