    ...
    def model_test(self, obs_data,interpolation_input_,geo_model_test,mean_init,cov_init,factor,num_layers,posterior_condition, scale, cluster, alpha, beta, dtype, device):
            ...
            if posterior_condition==1:
                ...
                for i in range(z_nk.shape[1]):
                    ...
                    mean.append(mean_k)
                    cov_k=cov_k/N_k[i] #+ 1e-3 * torch.diag(torch.ones(cov_k.shape[0],dtype=dtype, device=device))
                    cov.append(cov_k)
            ...
            if posterior_condition==2:
                ...
                for i in range(loc_mean.shape[0]):
                    cov_k = torch.zeros((loc_mean.shape[1],loc_mean.shape[1]),dtype=dtype, device=device)
                    for j in range(z_nk.shape[0]):
            ...
            if posterior_condition==3:
                ...
                for i in range(loc_mean.shape[0]):
                    ...
                    cov_data = eigen_vectors_data @ (torch.diag(cov_eigen_values)**2 + 1e-8 * torch.eye(cov_eigen_values.shape[0], dtype=dtype, device=device)) @ eigen_vectors_data.T #+ 1e-6 * torch.eye(loc_mean[0].shape[0], dtype=dtype, device=device)
                    cov.append(cov_data)
            ...
            # gmm_prior = torch.exp(
            #             dist.MultivariateNormal(loc=mean_tensor, covariance_matrix=cov_tensor).log_prob(obs_data.unsqueeze(1)))
            ...
            # pyro.factor("log_likelihood", log_likelihood_full)
