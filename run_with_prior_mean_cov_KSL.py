...
parser.add_argument('--directory_path', metavar='directory_path', type=str , default="./Results_with_prior_mean_cov_KSL", help='name of the directory in which result should be stored')
...
def main():
    ...
    if dimred=="pca":
        ...
        pca = PCA(n_components=0.99) # 99.0% of information is captured
   ...
   ################################################################################
    ...
    def model_test(obs_data):
        ...
        for i in range(loc_mean.shape[0]):
            ...
            cov_data = eigen_vectors_data @ torch.diag(cov_eigen_values)**2 @ eigen_vectors_data.T #+ 1e-8 * torch.eye(loc_mean[0].shape[0], dtype=torch.float64)
            cov.append(cov_data)
    ...
    az.plot_trace(data)
    filename_posterior_plot = directory_path + "/posterior.png"
    plt.savefig(filename_posterior_plot)
    plt.close()
    ###############################################TODO################################
    ...
    for i in range(posterior_samples["mu_1"].shape[0]):
        ...
        post_cov_eigen_values_4 = posterior_samples[keys_list[3]][i]
        post_mean_data_1 = posterior_samples[keys_list[4]][i]
        post_mean_data_2 = posterior_samples[keys_list[5]][i]
        post_mean_data_3 = posterior_samples[keys_list[6]][i]
        post_mean_data_4 = posterior_samples[keys_list[7]][i]
        post_mu_1 = posterior_samples[keys_list[8]][i]
        post_mu_2 = posterior_samples[keys_list[9]][i]
        post_mu_3 = posterior_samples[keys_list[10]][i]
        ...
        for j in range(loc_mean.shape[0]):
            log_prior_hsi_mean = log_prior_hsi_mean + dist.MultivariateNormal(loc=loc_mean[j],covariance_matrix=cov_matrix_mean).log_prob(posterior_samples[keys_list[4+j]][i])
        ...
        for j in range(normalised_hsi.shape[0]):
            likelihood = pi_k[0] *torch.exp(dist.MultivariateNormal(loc=post_mean_data_1,covariance_matrix= cov_tensor[0]).log_prob(normalised_hsi[j])) +\
                         ...
                         pi_k[2] *torch.exp(dist.MultivariateNormal(loc=post_mean_data_4,covariance_matrix= cov_tensor[3]).log_prob(normalised_hsi[j]))
            for k in range(loc_mean.shape[0]) :        
                gamma_nk[j][k] = (pi_k[k] * torch.exp(dist.MultivariateNormal(loc=posterior_samples[keys_list[4+k]][i],covariance_matrix= cov_tensor[k]).log_prob(normalised_hsi[j]))) / likelihood
    ...
    filename_final_sp = directory_path + "/Final_sp_mode.csv"
    ...
    for j in range(normalised_hsi.shape[0]):
            likelihood = pi_k[0] *torch.exp(dist.MultivariateNormal(loc=mean_tensor[0],covariance_matrix= cov_tensor[0]).log_prob(normalised_hsi[j])) +\
                         ...
                         pi_k[3] *torch.exp(dist.MultivariateNormal(loc=mean_tensor[3],covariance_matrix= cov_tensor[3]).log_prob(normalised_hsi[j]))
