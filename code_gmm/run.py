...
parser.add_argument('--posterior_number_samples', metavar='posterior_number_samples', type=int , default=400, help='number of samples for posterior')
...
parser.add_argument('--alpha', metavar='alpha', type=float , default=1e1, help='scaling parameter for the mean, 0.1')
...
def main():
    ...
    # Posterior 
    ################################################################################
    # print("Checking the pickle issue")
    # pickle.dumps(model.model_test(normalised_hsi,test_list,geo_model_test,mean_init,cov_init,factor,num_layers,posterior_condition,scale, cluster, alpha, beta,dtype,device))
        
    
    pyro.primitives.enable_validation(is_validate=True)
    nuts_kernel = NUTS(model.model_test, step_size=0.0085, adapt_step_size=True, target_accept_prob=0.7, max_tree_depth=10, init_strategy=init_to_mean)
    #nuts_kernel = NUTS(model.model_test,init_strategy=init_to_mean)
    mcmc = MCMC(nuts_kernel, num_samples=posterior_number_samples, warmup_steps=posterior_warmup_steps,num_chains=posterior_num_chain, mp_context = "fork", disable_validation=False)
    ...
    #########################################################################################
        ...
        for idx, (key, value) in enumerate(RV_mean_data_post_MAP.items()):
            ...
            cov_data = eigen_vectors_data @ (torch.diag(RV_post_cov_eigen_MAP[f"cov_eigen_values_{idx+1}"+"_post"])**2 +1e-8 * torch.eye(eigen_values_list[0].shape[0], dtype=dtype, device=device)) @ eigen_vectors_data.T
            mean.append(mean_k)
            cov.append(cov_data)
    ...
    #########################################################################################
        ...
        for idx, (key, value) in enumerate(RV_mean_data_post_Mean.items()):
            mean_k = value
            eigen_vectors_data = torch.tensor(eigen_vector_list[idx], dtype=dtype, device=device)
            cov_data = eigen_vectors_data @ (torch.diag(RV_post_cov_eigen_Mean[f"cov_eigen_values_{idx+1}"+"_post"])**2 + 1e-8 * torch.eye(eigen_values_list[0].shape[0], dtype=dtype, device=device)) @ eigen_vectors_data.T
            mean.append(mean_k)
            cov.append(cov_data)
