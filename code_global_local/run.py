...
parser.add_argument('--alpha', metavar='alpha', type=float , default=1e10, help='scaling parameter for the mean, 0.1')
parser.add_argument('--beta', metavar='beta', type=float , default=1e6, help='scaling parameter for the covariance, 20')
parser.add_argument('--delta',metavar='delta', type=int , default=4 * 1e-4, help='Global structure of Gaussian is close to GMM weights')
...
def main():
    ...
    if posterior_condition==1:
        directory_path = directory_path + "/" + dataset + "/posterior_condition_" + str(posterior_condition) + "_slope_gempy_" + str(slope_gempy) + "_delta_"+ str(delta) +"scale_" + str(scale)
    elif posterior_condition==2:
        directory_path = directory_path + "/" + dataset + "/posterior_condition_" + str(posterior_condition) + "_alpha_" + str(alpha) + "_slope_gempy_" + "_delta_"+ str(delta) + str(slope_gempy) + "scale_" + str(scale)
    elif (posterior_condition==3) or (posterior_condition==4):
        directory_path = directory_path + "/" + dataset + "/posterior_condition_" + str(posterior_condition) + "_alpha_" + str(alpha) + "_beta_"+str(beta) + "_delta_"+ str(delta) + "_slope_gempy_" + str(slope_gempy) + "scale_" + str(scale)
    ...
    if dataset=="Salinas":
        ...
        test_list.append({"update":"interface_data","id":torch.tensor([1]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[1,2],dtype=torch.float64), "std":torch.tensor(0.02,dtype=torch.float64)}})
        test_list.append({"update":"interface_data","id":torch.tensor([4]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[4,2],dtype=torch.float64), "std":torch.tensor(0.02,dtype=torch.float64)}})
        test_list.append({"update":"interface_data","id":torch.tensor([7]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[7,2],dtype=torch.float64), "std":torch.tensor(0.02,dtype=torch.float64)}})
    elif dataset=="KSL_layer3":
        test_list.append({"update":"interface_data","id":torch.tensor([1]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[1,2],dtype=torch.float64), "std":torch.tensor(0.06,dtype=torch.float64)}})
        test_list.append({"update":"interface_data","id":torch.tensor([4]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[4,2],dtype=torch.float64), "std":torch.tensor(0.06,dtype=torch.float64)}})
    ...
    dot = pyro.render_model(model.model_test, model_args=(normalised_hsi,test_list,geo_model_test, weights_init, mean_init,cov_init,factor,num_layers,posterior_condition, scale, cluster, alpha, beta, delta),render_distributions=True,filename=filename_Bayesian_graph)
    
    ################################################################################
    # Prior
    ################################################################################
    pyro.set_rng_seed(42)
    prior = Predictive(model.model_test, num_samples=prior_number_samples)(normalised_hsi,test_list,geo_model_test,weights_init,mean_init,cov_init,factor,num_layers,posterior_condition, scale, cluster, alpha, beta, delta)
    # Key to avoid
    avoid_key =[]
    ...
    mcmc.run(normalised_hsi,test_list,geo_model_test,weights_init, mean_init,cov_init,factor,num_layers,posterior_condition,scale, cluster, alpha, beta, delta)
    ...
    posterior_predictive = Predictive(model.model_test, posterior_samples)(normalised_hsi,test_list,geo_model_test,weights_init, mean_init,cov_init,factor,num_layers,posterior_condition,scale, cluster, alpha, beta, delta)
    plt.figure(figsize=(8,10))
    ...
    MAP_sample_index, max_posterior_value = compute_map(posterior_samples,geo_model_test,normalised_hsi,test_list,y_obs_label, weights_init, mean_init,cov_init,directory_path,num_layers,posterior_condition,scale, cluster, alpha, beta, delta)
