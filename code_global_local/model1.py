...
class MyModel(PyroModule):
    ...
    @config_enumerate
    def model_test(self, obs_data,interpolation_input_,geo_model_test, weights_init, mean_init, cov_init,factor,num_layers,posterior_condition, scale, cluster, alpha, beta, delta):
            """
            ...
            weights_init: initial weights from gmm
            ...
            if posterior_condition==3:
                ...
                for i in range(loc_mean.shape[0]):
                    ...
                    #cov_eigen_values = pyro.sample("cov_eigen_values_"+str(i+1), dist.MultivariateNormal(loc=torch.zeros(eigen_values_init.shape[0],dtype=torch.float64),covariance_matrix=cov_matrix_cov))
            ...
            with pyro.plate("K"+str(pi_k.shape[0]), pi_k.shape[0]):
                pyro.sample("weight", dist.Normal(loc=pi_k, scale = delta), obs=weights_init)
