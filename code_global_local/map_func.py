...
def compute_map(posterior_samples,geo_model_test,normalised_hsi,test_list,y_obs_label,weights_init, mean_init,cov_init,directory_path,num_layers,posterior_condition,scale, cluster, alpha, beta, delta):
    """
    ...
    weights_init = torch.tensor(weights_init, dtype=torch.float64)
    ...
    if(posterior_condition==1):
        ...
        for i in range(posterior_samples["mu_1"].shape[0]):
            ...
            for j in range(normalised_hsi.shape[0]):
                ...
                for k in range(len(pi_k)):
                    likelihood += z_nk[j,k] * torch.exp(dist.MultivariateNormal(loc=mean_tensor[k], covariance_matrix=cov_tensor[k]).log_prob(normalised_hsi[j]))            
                for k in range(len(pi_k)):
                    gamma_nk[j][k] = (z_nk[j,k] * torch.exp(dist.MultivariateNormal(loc=mean_tensor[k],covariance_matrix= cov_tensor[k]).log_prob(normalised_hsi[j]))) / likelihood
                    
                log_likelihood += torch.log(likelihood)
                
            # adding terms for global structure
            for j in range(pi_k.shape[0]):
                log_data = torch.exp(dist.Normal(loc=pi_k[j],scale=delta).log_prob(weights_init[j]))
                log_likelihood += torch.log(log_data)
        ...
        for i in range(posterior_samples[keys_list[0]].shape[0]):
            ...
            for j in range(loc_mean.shape[0]):
                ...
                #log_prior_hsi_cov = log_prior_hsi_cov + dist.MultivariateNormal(loc=torch.zeros(eigen_values_init.shape[0],dtype=torch.float64),covariance_matrix=cov_matrix_cov).log_prob(posterior_samples[keys_list[j]][i])
