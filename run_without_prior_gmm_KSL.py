...
def create_initial_gempy_model(refinement,save=True):
    ...
    if save:
        plt.savefig("./Results_without_prior_gmm_KSL/Prior_model.png")
...
def main():
    ...
    df_sp_init.to_csv("./Results_without_prior_gmm_KSL/Initial_sp.csv")
    df_or_init.to_csv("./Results_without_prior_gmm_KSL/Initial_or.csv")
    ################################################################################
    ...
    #######################################################################################
    if plot_dimred =="tsne":
        TSNE_transformation(data=normalised_hsi, label=gmm_label_rearranged, filename="./Results_without_prior_gmm_KSL/tsne_gmm_label.png")
    ...
    def model_test(obs_data):
        ...
        mu_surface_1 = pyro.sample('mu_1', dist.Normal(prior_mean_surface_1, torch.tensor(0.04, dtype=torch.float64)))
        mu_surface_2 = pyro.sample('mu_2', dist.Normal(prior_mean_surface_2, torch.tensor(0.04, dtype=torch.float64)))
    ...
    plt.savefig("./Results_without_prior_gmm_KSL/prior.png")
    ...
    az.plot_trace(data)
    plt.savefig("./Results_without_prior_gmm_KSL/posterior.png")
    ...
    plt.savefig("./Results_without_prior_gmm_KSL/mu_1.png")
    ...
    plt.savefig("./Results_without_prior_gmm_KSL/mu_2.png")
    ...
    for i in range(posterior_samples["mu_1"].shape[0]):
        ...
        log_prior_geo = dist.Normal(prior_mean_surface_1, torch.tensor(0.4, dtype=torch.float64)).log_prob(post_mu_1)+\
                    dist.Normal(prior_mean_surface_2, torch.tensor(0.4, dtype=torch.float64)).log_prob(post_mu_2)
    ...
    df_sp_final.to_csv("./Results_without_prior_gmm_KSL/Final_sp.csv")
    ################################################################################
    ...
    # loc_mean = torch.tensor(mean_init,dtype=torch.float64)
    # loc_cov =  torch.tensor(cov_init, dtype=torch.float64)
    z_nk = F.softmax(-lambda_* (torch.tensor([1,2,3], dtype=torch.float64) - torch.tensor(custom_grid_values_post).reshape(-1,1))**2, dim=1)
    #class_label = torch.mean(F.softmax(-lambda_* (torch.tensor([1,2,3,4,5,6], dtype=torch.float64) - custom_grid_values.reshape(-1,1))**2, dim=1),dim=0)
    ...
    # Save to file
    with open("./Results_without_prior_gmm_KSL/gmm_data.json", "w") as json_file:
        json.dump(gmm_data, json_file)
    ##########################################################################################
    
    picture_test_post = gpv.plot_2d(geo_model_test_post, cell_number=5, legend='force')
    plt.savefig("./Results_without_prior_gmm_KSL/Posterior_model.png")
    ...
    if plot_dimred=="tsne":
        ...
        TSNE_transformation(data=normalised_hsi, label= np.round(custom_grid_values_post), filename="./Results_without_prior_gmm_KSL/tsne_gempy_final_label.png")
